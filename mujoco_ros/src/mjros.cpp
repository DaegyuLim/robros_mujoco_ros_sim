...
void state_publisher()
{
    ...
    else
    {
        ...
        std::copy(d->qpos + 7, d->qpos + 14, mj_shm_->pos);
        std::copy(d->qvel + 6, d->qvel + 13, mj_shm_->vel);
        std::copy(d->qacc + 6, d->qacc + 13, mj_shm_->torqueActual);
    ...
    if (settings.run)
    {
        if (ros_sim_started)
        {
            if (mycontroller_tick_ % 10 == 0)
            {
                ...
                {
                    std::cout << "send states and update command torque" << std::endl;
                }
            }
